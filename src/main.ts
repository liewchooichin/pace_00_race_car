 ...
 */
const road = document.querySelector("#road")  as HTMLImageElement;
road.width = 1200;

const myBanner = document.querySelector("#myBanner") as HTMLHeadingElement;
const car = document.querySelector("#car") as HTMLImageElement;

/**
 * Convert style.left from integer xxx to "xxx.px".
 */
function convertIntToPx(pos: number){
...
function test_convertIntToPx(){
  const test_pos = 200;
  console.assert(convertIntToPx(test_pos)==="200px",
    "Fail: int to px conversion."
  )
}
test_convertIntToPx();
...
function moveCar(){
  ...
  const rightmost = Math.min(
    (road.width - car.width - 10), (window.innerWidth - car.width - 10)
  );
 ...
 * Fun
 */
const mainBody = document.querySelector("#main_body") as HTMLBodyElement;
mainBody.addEventListener("click",
  (event) => {console.log("Welcome to the race.");},
  false
)
